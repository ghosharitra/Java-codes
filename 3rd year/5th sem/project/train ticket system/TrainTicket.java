...
public class TrainTicket{
	public static void main(String ar []){
		...
		String sStationName,dStationName;
		sStationName = s.nextLine();
		dStationName = s.nextLine();
		price = station.price(sStationName,dStationName);
		...
		*/
		...
		for(int i = 0; i<16; i++){
			if (i<4){
				...
				else if (destination == dumToBally[i][0]){
			...
			if (i<8){
				if (source == ballyToBudge[i][0]){
					sourcePosition = i;
					listName1 = 1;
				}
				else if (destination == ballyToBudge[i][0]){
					destinationPosition = i;
					listName2 = 1;
				}
				...
				if (source == ballyToCan[i][0]){
					sourcePosition = i;
					listName1 = 2;
				}
				else if (destination == ballyToCan[i][0]){
					destinationPosition = i;
					listName2 = 2;
				}
			/*	
...
class NonAdmin extends Login{
	
	protected void ticketInfo(){
